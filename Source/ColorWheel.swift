...
class ColorWheel: UIView {
    var color: UIColor!
    //Edited
    static var imageStack: NSMutableArray = NSMutableArray();
    ...
    func createColorWheel(_ size: CGSize) -> CGImage {
        ...
        for storedObject in ColorWheel.imageStack {
            switch storedObject {
            case let cgimage as CGImage:
                if(cgimage.width == Int(dimension)) {
                    return cgimage
                }
            default:
                print("Didn't store image in imageStack smh")
            }
        ...
        let bitmapData: CFMutableData = CFDataCreateMutable(kCFAllocatorDefault, 0)
        ...
        ColorWheel.imageStack.add(imageRef)
        return imageRef!
    }
